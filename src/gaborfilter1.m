...
function [G,gabout] = gaborfilter1(I,Sx,Sy,f,theta,downsample)
...
gabout1 = sqrt(Imgabout.*Imgabout + Regabout.*Regabout);
[h,w]=size(gabout1);
gabout =ones(h/downsample, w/downsample);
for i=1:size(gabout,1)
    for j=1:size(gabout,2)
           gabout(i,j)=gabout1(i*downsample-uint16(downsample/2)+1,j*downsample-uint16(downsample/2)+1);
    end
end